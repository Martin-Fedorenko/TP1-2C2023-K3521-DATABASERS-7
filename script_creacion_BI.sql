...
BEGIN 
	...
	SELECT BI_TIEMPO_CODIGO, BI_ESTADO_ALQUILER_CODIGO, COUNT(*), SUM(CASE WHEN PAGO_ACTUAL.PAGO_ALQUILER_FECHA >= PAGO_ACTUAL.PAGO_ALQUILER_FECHA_VENCIMIENTO THEN 1 ELSE 0 END)
	--SUM((PAGO_ACTUAL.PAGO_ALQUILER_IMPORTE - PAGO_ANTERIOR.PAGO_ALQUILER_IMPORTE)/ PAGO_ANTERIOR.PAGO_ALQUILER_IMPORTE*100),
	--COUNT(PAGO_ANTERIOR.PAGO_ALQUILER_CODIGO)
	...
	JOIN DATABASERS.BI_TIEMPO ON BI_TIEMPO_MES=MONTH(PAGO_ACTUAL.PAGO_ALQUILER_FECHA) AND BI_TIEMPO_CUATRIMESTRE=DATEPART(QUARTER,PAGO_ACTUAL.PAGO_ALQUILER_FECHA) AND BI_TIEMPO_ANIO = YEAR(PAGO_ACTUAL.PAGO_ALQUILER_FECHA)
	JOIN DATABASERS.PAGO_ALQUILER PAGO_ANTERIOR ON PAGO_ACTUAL.PAGO_ALQUILER_ALQUILER=PAGO_ANTERIOR.PAGO_ALQUILER_ALQUILER 
	JOIN DATABASERS.BI_TIEMPO T2 ON T2.BI_TIEMPO_MES=DATEADD(MONTH,-1,PAGO_ACTUAL.PAGO_ALQUILER_FECHA) AND BI_TIEMPO_CUATRIMESTRE=DATEPART(QUARTER,PAGO_ANTERIOR.PAGO_ALQUILER_FECHA) AND BI_TIEMPO_ANIO = YEAR(PAGO_ANTERIOR.PAGO_ALQUILER_FECHA)
	GROUP BY BI_TIEMPO_CODIGO,  BI_ESTADO_ALQUILER_CODIGO
...
GO
-- VISTA 5 
CREATE VIEW DATABASERS.INCREMENTO_ALQUILER -- no revisada
AS
	SELECT CONCAT(SUM(BI_PAGO_ALQUILER_INCREMENTO)/SUM(BI_PAGO_ALQUILER_CANT_INCREMENTO), ' %') AS PORCENTAJE_PROM_INCREMENTO, BI_TIEMPO_ANIO, BI_TIEMPO_MES, BI_PAGO_ALQUILER_ESTADO_ALQUILER
	FROM DATABASERS.BI_PAGO_ALQUILER
	JOIN DATABASERS.BI_TIEMPO ON BI_PAGO_ALQUILER_TIEMPO = BI_TIEMPO_CODIGO
	GROUP BY BI_TIEMPO_ANIO, BI_TIEMPO_MES, BI_PAGO_ALQUILER_ESTADO_ALQUILER
