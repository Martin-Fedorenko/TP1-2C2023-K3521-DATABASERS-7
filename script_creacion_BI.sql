...
AS
BEGIN
DELETE FROM DATABASERS.BI_ANUNCIO
...
DELETE FROM DATABASERS.BI_TIEMPO
...
BEGIN
	...
	(BI_ANUNCIO_TIEMPO, BI_ANUNCIO_TIPO_OPERACION, BI_ANUNCIO_BARRIO, BI_ANUNCIO_AMBIENTES_INMUEBLE, BI_ANUNCIO_SUCURSAL, BI_ANUNCIO_TIPO_INMUEBLE, BI_ANUNCIO_MONEDA,  BI_ANUNCIO_RANGO_SUPERFICIE, BI_ANUNCIO_RANGO_ETARIO, BI_ANUNCIO_DIAS_PUBLICADO)
	SELECT BI_TIEMPO_CODIGO, ANUNCIO_TIPO_OPERACION, INMUEBLE_BARRIO, INMUEBLE_CANT_AMBIENTES, ANUNCIO_SUCURSAL, INMUEBLE_TIPO_INMUEBLE, ANUNCIO_MONEDA, DATABASERS.OBTENER_RANGO_SUPERFICIE(INMUEBLE_SUPERFICIETOTAL),  DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE()), AVG(DATEDIFF(DAY, ANUNCIO_FECHA_PUBLICACION, ANUNCIO_FECHA_FINALIZACION)), AVG(ANUNCIO_PRECIO_PUBLICADO)
...
BEGIN 
	...
	(BI_ALQUILER_BARRIO,BI_ALQUILER_TIEMPO,BI_ALQUILER_RANGO_ETARIO)
	SELECT DISTINCT INMUEBLE_BARRIO,BI_TIEMPO_CODIGO, DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, PAGO_ALQUILER_FECHA)
...
BEGIN 
	...
	SELECT DISTINCT BI_TIEMPO_CODIGO,CASE PAGO_ALQUILER_FECHA >= PAGO_ALQUILER_FECHA_VENCIMIENTO THEN 1 ELSE 0 END, FROM DATABASERS.ALQUILER 
	JOIN DATABASERS.PAGO_ALQUILER ON PAGO_ALQUILER_ALQUILER=ALQUILER_CODIGO 
...
SELECT * FROM DATABASERS.ALQUILER WHERE ALQUILER_COMISION IS NULL
select * from DATABASERS.ANUNCIO --22397
select * from DATABASERS.INMUEBLE --8152
select * from DATABASERS.PERSONA --25055
select * from DATABASERS.BI_TIEMPO --10
...
BEGIN
	...
	begin 
			...
			BEGIN
				SET @MONTO_TOTAL_OPERACION = (SELECT VENTA_PRECIO_VENTA + VENTA_COMISION FROM DATABASERS.VENTA WHERE VENTA_ANUNCIO = @ANUNCIO_CODIGO) + (SELECT VENTA_COMISION FROM DATABASERS.VENTA WHERE VENTA_ANUNCIO = @ANUNCIO_CODIGO)
...
-- VISTA 1
CREATE VIEW DATABASERS.PROM_DIAS_PUBLICACION --1434 AVG EN VISTA, 1434 AVG EN MIGRACION
AS
...
AS
	SELECT BI_ANUNCIO_COMISION AS COMISION_PROMEDIO, BI_TIPO_OPERACION_DETALLE, BI_SUCURSAL_NOMBRE, BI_TIEMPO_CUATRIMESTRE, BI_TIEMPO_ANIO
	FROM DATABASERS.BI_ANUNCIO 
	JOIN DATABASERS.BI_TIPO_OPERACION ON BI_ANUNCIO_TIPO_OPERACION = BI_TIPO_OPERACION_CODIGO
	JOIN DATABASERS.BI_SUCURSAL ON BI_ANUNCIO_SUCURSAL = BI_SUCURSAL_CODIGO
	JOIN DATABASERS.BI_TIEMPO ON BI_ANUNCIO_TIEMPO = BI_TIEMPO_CODIGO
	GROUP BY BI_ANUNCIO_COMISION, BI_TIPO_OPERACION_DETALLE, BI_SUCURSAL_NOMBRE, BI_TIEMPO_CUATRIMESTRE, BI_TIEMPO_ANIO
GO
