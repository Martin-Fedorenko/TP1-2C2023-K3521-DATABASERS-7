...
-- VISTA 2
CREATE VIEW DATABASERS.PROM_PRECIO_ANUNCIO -- no revisada
AS
...
AS
	SELECT CONCAT(
	(CASE WHEN SUM(BI_PAGO_ALQUILER_CANT_INCREMENTO) = 0 THEN 0 ELSE 
	SUM(BI_PAGO_ALQUILER_INCREMENTO)/SUM(BI_PAGO_ALQUILER_CANT_INCREMENTO) END)
	, ' %') AS PORCENTAJE_PROM_INCREMENTO, BI_TIEMPO_ANIO, BI_TIEMPO_MES, BI_ESTADO_ALQUILER_DETALLE
	FROM DATABASERS.BI_PAGO_ALQUILER
	JOIN DATABASERS.BI_TIEMPO ON BI_PAGO_ALQUILER_TIEMPO = BI_TIEMPO_CODIGO
...
SELECT * FROM DATABASERS.BI_PAGO_ALQUILER
