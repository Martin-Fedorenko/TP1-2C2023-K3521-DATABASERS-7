...
CREATE TABLE DATABASERS.BI_UBICACION (
	BI_UBICACION_CODIGO NUMERIC(18,0) PRIMARY KEY NOT NULL,
...
BEGIN
	...
	SUM(ANUNCIO_PRECIO_PUBLICADO),
	(CASE WHEN ANUNCIO_ESTADO = 2 THEN 0 WHEN ANUNCIO_ESTADO = 1 THEN SUM(VENTA_COMISION) WHEN ANUNCIO_ESTADO = 3 THEN SUM(ALQUILER_COMISION) END),
	(CASE WHEN ANUNCIO_ESTADO = 2 THEN 0 WHEN ANUNCIO_ESTADO = 1 THEN SUM(VENTA_PRECIO_VENTA+VENTA_COMISION) WHEN ANUNCIO_ESTADO = 3 THEN SUM(ALQUILER_COMISION+ALQUILER_GASTOS_AVERIGUA+ALQUILER_DEPOSITO) END),
	(CASE WHEN ANUNCIO_ESTADO = 2 THEN 0 WHEN ANUNCIO_ESTADO = 1 THEN COUNT(*) WHEN ANUNCIO_ESTADO = 3 THEN COUNT(*) END),
	(SELECT COUNT(*) FROM DATABASERS.ANUNCIO)
	...
	LEFT JOIN DATABASERS.VENTA ON ANUNCIO_CODIGO = VENTA_ANUNCIO
	LEFT JOIN DATABASERS.ALQUILER ON ANUNCIO_CODIGO = ALQUILER_ANUNCIO
	...
	SELECT BI_TIEMPO_CODIGO, ANUNCIO_TIPO_OPERACION, INMUEBLE_BARRIO, INMUEBLE_CANT_AMBIENTES, ANUNCIO_SUCURSAL, --4054
	INMUEBLE_TIPO_INMUEBLE, ANUNCIO_MONEDA, DATABASERS.OBTENER_RANGO_SUPERFICIE(INMUEBLE_SUPERFICIETOTAL),
	DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE()), SUM(DATEDIFF(DAY, ANUNCIO_FECHA_PUBLICACION, ANUNCIO_FECHA_FINALIZACION)),
	SUM(ANUNCIO_PRECIO_PUBLICADO), 0, 0, 0, (SELECT COUNT(*) FROM DATABASERS.ANUNCIO)
	FROM DATABASERS.ANUNCIO A
	JOIN DATABASERS.INMUEBLE ON ANUNCIO_INMUEBLE = INMUEBLE_CODIGO
	JOIN DATABASERS.PERSONA ON ANUNCIO_AGENTE = PERSONA_CODIGO
	JOIN DATABASERS.BI_TIEMPO ON BI_TIEMPO_MES = MONTH(ANUNCIO_FECHA_PUBLICACION) AND BI_TIEMPO_CUATRIMESTRE = DATEPART(QUARTER, ANUNCIO_FECHA_PUBLICACION) AND BI_TIEMPO_ANIO = YEAR(ANUNCIO_FECHA_PUBLICACION)
	WHERE ANUNCIO_ESTADO = 2
	GROUP BY BI_TIEMPO_CODIGO, BI_TIEMPO_ANIO, BI_TIEMPO_CUATRIMESTRE, ANUNCIO_TIPO_OPERACION, INMUEBLE_BARRIO, INMUEBLE_CANT_AMBIENTES, ANUNCIO_SUCURSAL, INMUEBLE_TIPO_INMUEBLE, ANUNCIO_MONEDA, DATABASERS.OBTENER_RANGO_SUPERFICIE(INMUEBLE_SUPERFICIETOTAL),  DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE())
	UNION
	--anuncios con ventas
	/*
	INSERT INTO DATABASERS.BI_ANUNCIO 
	...
	BI_ANUNCIO_DIAS_PUBLICADO, BI_ANUNCIO_PRECIO_INMUEBLE, BI_ANUNCIO_COMISION, BI_ANUNCIO_MONTO_TOTAL, BI_ANUNCIO_OP_CONCRETADA, BI_ANUNCIO_CANTIDAD_ANUNCIOS)
	*/	
	SELECT BI_TIEMPO_CODIGO, ANUNCIO_TIPO_OPERACION, INMUEBLE_BARRIO, INMUEBLE_CANT_AMBIENTES, ANUNCIO_SUCURSAL, --4055
	INMUEBLE_TIPO_INMUEBLE, ANUNCIO_MONEDA, DATABASERS.OBTENER_RANGO_SUPERFICIE(INMUEBLE_SUPERFICIETOTAL),
	DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE()), SUM(DATEDIFF(DAY, ANUNCIO_FECHA_PUBLICACION, ANUNCIO_FECHA_FINALIZACION)),
	SUM(ANUNCIO_PRECIO_PUBLICADO), SUM(VENTA_COMISION), SUM(VENTA_PRECIO_VENTA+VENTA_COMISION), COUNT(*), (SELECT COUNT(*) FROM DATABASERS.ANUNCIO)
	FROM DATABASERS.ANUNCIO A
	...
	BI_ANUNCIO_DIAS_PUBLICADO, BI_ANUNCIO_PRECIO_INMUEBLE, BI_ANUNCIO_COMISION, BI_ANUNCIO_MONTO_TOTAL, BI_ANUNCIO_OP_CONCRETADA, BI_ANUNCIO_CANTIDAD_ANUNCIOS)
	*/	
	SELECT BI_TIEMPO_CODIGO, ANUNCIO_TIPO_OPERACION, INMUEBLE_BARRIO, INMUEBLE_CANT_AMBIENTES, ANUNCIO_SUCURSAL, --11506
	INMUEBLE_TIPO_INMUEBLE, ANUNCIO_MONEDA, DATABASERS.OBTENER_RANGO_SUPERFICIE(INMUEBLE_SUPERFICIETOTAL),
	DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE()), SUM(DATEDIFF(DAY, ANUNCIO_FECHA_PUBLICACION, ANUNCIO_FECHA_FINALIZACION)),
	SUM(ANUNCIO_PRECIO_PUBLICADO), SUM(ALQUILER_COMISION), SUM(ALQUILER_COMISION+ALQUILER_GASTOS_AVERIGUA+ALQUILER_DEPOSITO), COUNT(*), (SELECT COUNT(*) FROM DATABASERS.ANUNCIO)
	FROM DATABASERS.ANUNCIO A
