...
AS
BEGIN
	IF OBJECT_ID('DATABASERS.MIGRAR_RELACIONAL', 'P') IS NOT NULL DROP PROCEDURE DATABASERS.MIGRAR_RELACIONAL
...
(
	...
	BI_ESTADO_ALQUILER_DETALLE NVARCHAR(100) NULL
)
...
(
	...
	BI_VENTA_CANT_TOTAL INTEGER NULL,
	BI_VENTA_PRECIO_M2 NUMERIC(18, 2) NULL,
	PRIMARY KEY(BI_VENTA_UBICACION, BI_VENTA_TIEMPO, BI_VENTA_TIPO_INMUEBLE)
)
...
(
	...
	BI_ALQUILER_CANTIDAD_ALQUILERES INTEGER NULL,	
	PRIMARY KEY(BI_ALQUILER_UBICACION, BI_ALQUILER_TIEMPO, BI_ALQUILER_RANGO_ETARIO)
)

CREATE TABLE DATABASERS.BI_PAGO_ALQUILER
(
	BI_PAGO_ALQUILER_TIEMPO INTEGER NOT NULL,
	BI_PAGO_ALQUILER_ESTADO_ALQUILER NUMERIC (18,0) NOT NULL,
	BI_PAGO_ALQUILER_CANT_PAGOS  INTEGER NULL,
	BI_PAGO_ALQUILER_CANT_INCLUMPLIDOS INTEGER NULL,
	BI_PAGO_ALQUILER_INCREMENTO NUMERIC(18,2) NULL,
	BI_PAGO_ALQUILER_CANT_INCREMENTO INTEGER NULL,
	PRIMARY KEY(BI_PAGO_ALQUILER_TIEMPO, BI_PAGO_ALQUILER_ESTADO_ALQUILER)
)
...
ALTER TABLE DATABASERS.BI_PAGO_ALQUILER
ADD FOREIGN KEY (BI_PAGO_ALQUILER_ESTADO_ALQUILER) REFERENCES DATABASERS.BI_ESTADO_ALQUILER(BI_ESTADO_ALQUILER_CODIGO);
...
BEGIN
	...
	--anuncios que no tuvieron ni ventas ni alquileres
	...
	SUM(ANUNCIO_PRECIO_PUBLICADO), 0, 0, 0, COUNT(*)
	...
	GROUP BY T1.BI_TIEMPO_CODIGO, ANUNCIO_TIPO_OPERACION, BI_UBICACION_CODIGO, INMUEBLE_CANT_AMBIENTES, ANUNCIO_SUCURSAL, INMUEBLE_TIPO_INMUEBLE, ANUNCIO_MONEDA, DATABASERS.OBTENER_RANGO_SUPERFICIE(INMUEBLE_SUPERFICIETOTAL),  DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE()), ANUNCIO_ESTADO
	...
	SELECT BI_TIEMPO_CODIGO, ANUNCIO_ESTADO, ANUNCIO_TIPO_OPERACION, BI_UBICACION_CODIGO, INMUEBLE_CANT_AMBIENTES, ANUNCIO_SUCURSAL,
	INMUEBLE_TIPO_INMUEBLE, ANUNCIO_MONEDA, DATABASERS.OBTENER_RANGO_SUPERFICIE(INMUEBLE_SUPERFICIETOTAL),
	DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE()), SUM(DATEDIFF(DAY, ANUNCIO_FECHA_PUBLICACION, ANUNCIO_FECHA_FINALIZACION)),
	SUM(ANUNCIO_PRECIO_PUBLICADO), SUM(VENTA_COMISION), SUM(VENTA_PRECIO_VENTA+VENTA_COMISION), COUNT(*), COUNT(*)
	FROM DATABASERS.ANUNCIO A
	...
	INMUEBLE_TIPO_INMUEBLE, ANUNCIO_MONEDA, DATABASERS.OBTENER_RANGO_SUPERFICIE(INMUEBLE_SUPERFICIETOTAL),
	DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE()), SUM(DATEDIFF(DAY, ANUNCIO_FECHA_PUBLICACION, ANUNCIO_FECHA_FINALIZACION)),
	SUM(ANUNCIO_PRECIO_PUBLICADO), SUM(ALQUILER_COMISION), SUM(ALQUILER_COMISION+ALQUILER_GASTOS_AVERIGUA+ALQUILER_DEPOSITO), COUNT(*), COUNT(*)
	FROM DATABASERS.ANUNCIO A
...
BEGIN 
	...
	(BI_ALQUILER_UBICACION, BI_ALQUILER_TIEMPO,BI_ALQUILER_RANGO_ETARIO, BI_ALQUILER_CANTIDAD_ALQUILERES)
	
	SELECT BI_UBICACION_CODIGO, BI_TIEMPO_CODIGO,  DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE()), COUNT(*)  FROM DATABASERS.ANUNCIO
	JOIN DATABASERS.ALQUILER ON ALQUILER_ANUNCIO = ANUNCIO_CODIGO
	JOIN DATABASERS.INMUEBLE ON INMUEBLE_CODIGO = ANUNCIO_INMUEBLE
	JOIN DATABASERS.INQUILINO ON INQUILINO_ALQUILER = ALQUILER_CODIGO
	...
	GROUP BY BI_UBICACION_CODIGO, BI_TIEMPO_CODIGO, DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE())
...
BEGIN 
	...
	(BI_PAGO_ALQUILER_TIEMPO, BI_PAGO_ALQUILER_ESTADO_ALQUILER, BI_PAGO_ALQUILER_CANT_PAGOS, BI_PAGO_ALQUILER_CANT_INCLUMPLIDOS, BI_PAGO_ALQUILER_INCREMENTO, BI_PAGO_ALQUILER_CANT_INCREMENTO)
	
	SELECT T1.BI_TIEMPO_CODIGO, BI_ESTADO_ALQUILER_CODIGO, COUNT(*), SUM(CASE WHEN PAGO_ACTUAL.PAGO_ALQUILER_FECHA >= PAGO_ACTUAL.PAGO_ALQUILER_FECHA_VENCIMIENTO THEN 1 ELSE 0 END),
	SUM(ISNULL((PAGO_ACTUAL.PAGO_ALQUILER_IMPORTE - PAGO_ANTERIOR.PAGO_ALQUILER_IMPORTE)/ PAGO_ANTERIOR.PAGO_ALQUILER_IMPORTE*100, 0)),
	SUM(CASE WHEN PAGO_ACTUAL.PAGO_ALQUILER_IMPORTE > PAGO_ANTERIOR.PAGO_ALQUILER_IMPORTE THEN 1 ELSE 0 END)

	FROM DATABASERS.ALQUILER 
	JOIN DATABASERS.BI_ESTADO_ALQUILER ON ALQUILER_ESTADO = BI_ESTADO_ALQUILER_CODIGO
	JOIN DATABASERS.PAGO_ALQUILER PAGO_ACTUAL ON PAGO_ACTUAL.PAGO_ALQUILER_ALQUILER=ALQUILER_CODIGO
	LEFT JOIN DATABASERS.PAGO_ALQUILER PAGO_ANTERIOR ON PAGO_ACTUAL.PAGO_ALQUILER_ALQUILER=PAGO_ANTERIOR.PAGO_ALQUILER_ALQUILER AND
	PAGO_ANTERIOR.PAGO_ALQUILER_FECHA = DATEADD(MONTH,-1,PAGO_ACTUAL.PAGO_ALQUILER_FECHA) AND 
	YEAR(PAGO_ANTERIOR.PAGO_ALQUILER_FECHA) = YEAR(PAGO_ACTUAL.PAGO_ALQUILER_FECHA) AND
	PAGO_ACTUAL.PAGO_ALQUILER_IMPORTE > PAGO_ANTERIOR.PAGO_ALQUILER_IMPORTE
	JOIN DATABASERS.BI_TIEMPO T1 ON T1.BI_TIEMPO_MES=MONTH(PAGO_ACTUAL.PAGO_ALQUILER_FECHA) AND T1.BI_TIEMPO_CUATRIMESTRE=DATEPART(QUARTER,PAGO_ACTUAL.PAGO_ALQUILER_FECHA) AND T1.BI_TIEMPO_ANIO = YEAR(PAGO_ACTUAL.PAGO_ALQUILER_FECHA)
	GROUP BY T1.BI_TIEMPO_CODIGO, BI_ESTADO_ALQUILER_CODIGO
...
BEGIN
    ...
    (BI_VENTA_UBICACION, BI_VENTA_TIEMPO, BI_VENTA_TIPO_INMUEBLE,  BI_VENTA_CANT_TOTAL, BI_VENTA_PRECIO_M2)
	 	
	SELECT BI_UBICACION_CODIGO, BI_TIEMPO_CODIGO, BI_TIPO_INMUEBLE_CODIGO, COUNT(*), SUM(VENTA_PRECIO_VENTA/INMUEBLE_SUPERFICIETOTAL)
	FROM DATABASERS.VENTA
	JOIN DATABASERS.ANUNCIO ON ANUNCIO_CODIGO = VENTA_ANUNCIO
	JOIN DATABASERS.INMUEBLE ON INMUEBLE_CODIGO = ANUNCIO_INMUEBLE
	JOIN DATABASERS.BI_TIPO_INMUEBLE ON BI_TIPO_INMUEBLE_CODIGO = INMUEBLE_TIPO_INMUEBLE
	JOIN DATABASERS.BARRIO ON INMUEBLE_BARRIO = BARRIO_CODIGO
	JOIN DATABASERS.LOCALIDAD ON BARRIO_LOCALIDAD = LOCALIDAD_CODIGO
	JOIN DATABASERS.BI_UBICACION ON BI_BARRIO_CODIGO = INMUEBLE_BARRIO AND BI_LOCALIDAD_CODIGO = BARRIO_LOCALIDAD AND BI_PROVINCIA_CODIGO  = LOCALIDAD_PROVINCIA
	JOIN DATABASERS.BI_TIEMPO ON BI_TIEMPO_MES=MONTH(VENTA_FECHA) AND BI_TIEMPO_CUATRIMESTRE=DATEPART(QUARTER,VENTA_FECHA) AND BI_TIEMPO_ANIO = YEAR(VENTA_FECHA)
	GROUP BY BI_UBICACION_CODIGO, BI_TIEMPO_CODIGO, BI_TIPO_INMUEBLE_CODIGO
...
AS
	SELECT SUM(BI_ANUNCIO_DIAS_PUBLICADO)/SUM(BI_ANUNCIO_CANTIDAD_ANUNCIOS) AS DIAS_PROMEDIO, BI_TIPO_OPERACION_DETALLE, BI_AMBIENTES_INMUEBLE_DETALLE, BI_BARRIO_DETALLE, BI_TIEMPO_CUATRIMESTRE, BI_TIEMPO_ANIO
	FROM DATABASERS.BI_ANUNCIO 
	JOIN DATABASERS.BI_TIPO_OPERACION ON BI_ANUNCIO_TIPO_OPERACION = BI_TIPO_OPERACION_CODIGO
	JOIN DATABASERS.BI_AMBIENTES_INMUEBLE ON BI_ANUNCIO_AMBIENTES_INMUEBLE = BI_AMBIENTES_INMUEBLE_CODIGO
	...
	GROUP BY BI_TIPO_OPERACION_DETALLE, BI_BARRIO_DETALLE, BI_TIEMPO_CUATRIMESTRE, BI_TIEMPO_ANIO, BI_AMBIENTES_INMUEBLE_DETALLE
GO

-- VISTA 2
CREATE VIEW DATABASERS.PROM_PRECIO_ANUNCIO
AS
	SELECT CONCAT(SUM(BI_ANUNCIO_PRECIO_INMUEBLE)/SUM(BI_ANUNCIO_CANTIDAD_ANUNCIOS),' ',BI_MONEDA_DETALLE) AS PRECIO_PROMEDIO, BI_TIPO_OPERACION_DETALLE, BI_TIPO_INMUEBLE_DETALLE, BI_RANGO_SUPERFICIE_DETALLE, BI_TIEMPO_CUATRIMESTRE, BI_TIEMPO_ANIO
	FROM DATABASERS.BI_ANUNCIO 
	JOIN DATABASERS.BI_TIPO_OPERACION ON BI_ANUNCIO_TIPO_OPERACION = BI_TIPO_OPERACION_CODIGO
...
AS
	SELECT TOP 5 BI_BARRIO_DETALLE, BI_TIEMPO_ANIO, BI_TIEMPO_CUATRIMESTRE, BI_RANGO_ETARIO_DETALLE, SUM(BI_ALQUILER_CANTIDAD_ALQUILERES) AS CANTIDAD
    ...
    JOIN DATABASERS.BI_UBICACION ON BI_ALQUILER_UBICACION = BI_UBICACION_CODIGO
	GROUP BY BI_BARRIO_DETALLE, BI_TIEMPO_ANIO, BI_TIEMPO_CUATRIMESTRE, BI_RANGO_ETARIO_DETALLE
	ORDER BY SUM(BI_ALQUILER_CANTIDAD_ALQUILERES) DESC
GO
...
AS
	SELECT CONCAT(100*SUM(BI_PAGO_ALQUILER_CANT_INCLUMPLIDOS)/SUM(BI_PAGO_ALQUILER_CANT_PAGOS), ' %') AS PORCENTAJE_INCUMPLIMIENTO, BI_TIEMPO_ANIO, BI_TIEMPO_MES
...
AS
	SELECT CONCAT(
	(CASE WHEN SUM(BI_PAGO_ALQUILER_CANT_INCREMENTO) = 0 THEN 0 ELSE SUM(BI_PAGO_ALQUILER_INCREMENTO)/SUM(BI_PAGO_ALQUILER_CANT_INCREMENTO) END), ' %') AS PORCENTAJE_PROM_INCREMENTO, 
	--SI LA SUMA DE CANTIDAD DE INCREMENTOS ES CERO, IMPLICA QUE NO HUBO INCREMENTOS PARA ESE CASO, POR LO TANTO EL PORCENTAJE ES CERO
	BI_TIEMPO_ANIO, BI_TIEMPO_MES, BI_ESTADO_ALQUILER_DETALLE
	FROM DATABASERS.BI_PAGO_ALQUILER
	JOIN DATABASERS.BI_TIEMPO ON BI_PAGO_ALQUILER_TIEMPO = BI_TIEMPO_CODIGO
	JOIN DATABASERS.BI_ESTADO_ALQUILER ON BI_ESTADO_ALQUILER_CODIGO = BI_PAGO_ALQUILER_ESTADO_ALQUILER
	WHERE BI_ESTADO_ALQUILER_DETALLE = 'Activo' --SE SELECCIONAN SOLO AQUELLOS ALQUILERES QUE ESTAN "ACTIVOS"
	GROUP BY BI_TIEMPO_ANIO, BI_TIEMPO_MES, BI_ESTADO_ALQUILER_DETALLE
GO

-- VISTA 6
CREATE VIEW DATABASERS.PROM_PRECIO_M2_VENTAS 
AS
	SELECT BI_TIPO_INMUEBLE_DETALLE, BI_LOCALIDAD_DETALLE, BI_TIEMPO_ANIO, BI_TIEMPO_CUATRIMESTRE, (SUM(BI_VENTA_PRECIO_M2)/SUM(BI_VENTA_CANT_TOTAL)) AS PROM_PRECIO_M2
	FROM DATABASERS.BI_VENTA 
	JOIN DATABASERS.BI_TIPO_INMUEBLE ON BI_VENTA_TIPO_INMUEBLE= BI_TIPO_INMUEBLE_CODIGO
	JOIN DATABASERS.BI_UBICACION ON BI_UBICACION_CODIGO = BI_VENTA_UBICACION
...
AS
	SELECT BI_SUCURSAL_NOMBRE, BI_RANGO_ETARIO_DETALLE, BI_TIPO_OPERACION_DETALLE, BI_TIEMPO_ANIO, CONCAT(CAST((100 * SUM(CAST(BI_ANUNCIO_OP_CONCRETADA AS DECIMAL)) / SUM(BI_ANUNCIO_CANTIDAD_ANUNCIOS)) AS DECIMAL(12,2)), ' %') AS PORCENTAJE 
	FROM DATABASERS.BI_ANUNCIO
	JOIN DATABASERS.BI_SUCURSAL ON BI_ANUNCIO_SUCURSAL = BI_SUCURSAL_CODIGO
	JOIN DATABASERS.BI_TIEMPO ON BI_ANUNCIO_TIEMPO = BI_TIEMPO_CODIGO
	JOIN DATABASERS.BI_RANGO_ETARIO ON BI_ANUNCIO_RANGO_ETARIO = BI_RANGO_ETARIO_CODIGO
	JOIN DATABASERS.BI_TIPO_OPERACION ON BI_ANUNCIO_TIPO_OPERACION = BI_TIPO_OPERACION_CODIGO
	GROUP BY BI_SUCURSAL_NOMBRE, BI_RANGO_ETARIO_DETALLE, BI_TIPO_OPERACION_DETALLE, BI_TIEMPO_ANIO
GO

--VISTA 9
CREATE VIEW DATABASERS.MONTO_TOTAL_CIERRE_CONTRATOS_POR_TIPO_OP
AS
	SELECT CONCAT(SUM(BI_ANUNCIO_MONTO_TOTAL),' ',BI_MONEDA_DETALLE) AS MONTO_TOTAL_CIERRE_CONTRATO, BI_TIEMPO_CUATRIMESTRE,BI_TIPO_OPERACION_DETALLE, BI_SUCURSAL_NOMBRE
	FROM DATABASERS.BI_ANUNCIO
	JOIN DATABASERS.BI_SUCURSAL ON BI_ANUNCIO_SUCURSAL = BI_SUCURSAL_CODIGO
	JOIN DATABASERS.BI_TIEMPO ON BI_ANUNCIO_TIEMPO = BI_TIEMPO_CODIGO
	JOIN DATABASERS.BI_MONEDA ON BI_ANUNCIO_MONEDA = BI_MONEDA_CODIGO
