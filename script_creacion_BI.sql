...
BEGIN 
	...
	(BI_ALQUILER_UBICACION, BI_ALQUILER_TIEMPO,BI_ALQUILER_RANGO_ETARIO)
	
	SELECT BI_UBICACION_CODIGO, BI_TIEMPO_CODIGO,  DATABASERS.OBTENER_RANGO_ETARIO(PERSONA_FECHA_NAC, GETDATE())  FROM DATABASERS.ANUNCIO
...
BEGIN 
	...
	(BI_PAGO_ALQUILER_TIEMPO,BI_PAGO_ALQUILER_CANT_PAGOS, BI_PAGO_ALQUILER_CANT_INCLUMPLIDOS ) --BI_PAGO_ALQUILER_INCREMENTO, BI_PAGO_ALQUILER_CANT_INCREMENTO
	SELECT BI_TIEMPO_CODIGO, 
	COUNT(*),
	SUM(CASE WHEN PAGO_ALQUILER_FECHA >= PAGO_ALQUILER_FECHA_VENCIMIENTO THEN 1 ELSE 0 END)
	--FALTAN COMPLETAR LOS DOS ULTIMOS CAMPOS
	...
	GROUP BY BI_TIEMPO_CODIGO, BI_TIEMPO_ANIO, BI_TIEMPO_MES
END 
...
SELECT * FROM (SELECT BI_BARRIO_DETALLE, BI_TIEMPO_ANIO, BI_TIEMPO_CUATRIMESTRE, BI_RANGO_ETARIO_DETALLE, COUNT(*) AS CANTIDAD,
ROW_NUMBER() OVER (PARTITION BY BI_TIEMPO_ANIO, BI_TIEMPO_CUATRIMESTRE, BI_RANGO_ETARIO_DETALLE ORDER BY COUNT(*) DESC) AS POSICION
...
-- VISTA 6
CREATE VIEW DATABASERS.PROM_PRECIO_M2_VENTAS -- no revisada
AS
