...
(
	...
	BI_ALQUILER_INCUMPLIMIENTO INTEGER NULL,
...
BEGIN
	...
	begin 
			...
			BEGIN
				SET @MONTO_TOTAL_OPERACION = (SELECT VENTA_PRECIO_VENTA FROM DATABASERS.VENTA WHERE VENTA_ANUNCIO = @ANUNCIO_CODIGO)
			...
			BEGIN
				SET @MONTO_TOTAL_OPERACION = (SELECT ALQUILER_DEPOSITO FROM DATABASERS.ALQUILER WHERE ALQUILER_ANUNCIO = @ANUNCIO_CODIGO)
				SET @OPERACION_CONCRETADA = 1
			END
			--
			...
			BI_ANUNCIO_OP_CONTRECTADAS, BI_ANUNCIO_MONTO_TOTAL)
...
BEGIN
	...
	begin 
			...
			IF(@ALQUILER_CODIGO_ANTERIOR = @ALQUILER_CODIGO)
				SET @PORCENTAJE = ((@ALQUILER_PAGO_IMPORTE - @ALQUILER_IMPORTE_ANTERIOR)/@ALQUILER_IMPORTE_ANTERIOR)*100
			ELSE
				SET @PORCENTAJE = 0
...
FROM DATABASERS.BI_ALQUILER
JOIN DATABASERS.BI_TIEMPO ON BI_ALQUILER_TIEMPO = BI_TIEMPO_CODIGO
WHERE BI_ALQUILER_ESTADO = 3 AND BI_ALQUILER_INCREMENTO != 0
GROUP BY BI_TIEMPO_ANIO, BI_TIEMPO_MES
GO
...
SELECT BI_SUCURSAL_NOMBRE, BI_RANGO_ETARIO_DETALLE, BI_TIEMPO_ANIO, COUNT(*) AS CANTIDAD_OPERACIONES_CONCRETADAS 
FROM DATABASERS.BI_ANUNCIO 
JOIN DATABASERS.BI_SUCURSAL ON BI_ANUNCIO_SUCURSAL = BI_SUCURSAL_CODIGO
JOIN DATABASERS.BI_TIEMPO ON BI_ANUNCIO_TIEMPO = BI_TIEMPO_CODIGO
JOIN DATABASERS.BI_RANGO_ETARIO ON BI_ANUNCIO_RANGO_ETARIO = BI_RANGO_ETARIO_CODIGO
WHERE BI_ANUNCIO_OP_CONTRECTADAS=1
GROUP BY BI_SUCURSAL_NOMBRE, BI_RANGO_ETARIO_DETALLE, BI_TIEMPO_ANIO
ORDER BY BI_TIEMPO_ANIO
...
SELECT DISTINCT ALQUILER_CODIGO, PAGO_ALQUILER_IMPORTE FROM DATABASERS.ALQUILER JOIN DATABASERS.PAGO_ALQUILER ON ALQUILER_CODIGO = PAGO_ALQUILER_ALQUILER ORDER BY ALQUILER_CODIGO ASC

SELECT ALQUILER_CODIGO FROM DATABASERS.ALQUILER JOIN DATABASERS.PAGO_ALQUILER ON ALQUILER_CODIGO = PAGO_ALQUILER_ALQUILER 
WHERE YEAR(PAGO_ALQUILER_FECHA) = 2024 AND MONTH(PAGO_ALQUILER_FECHA) = 1 AND PAGO_ALQUILER_FECHA >= PAGO_ALQUILER_FECHA_VENCIMIENTO

SELECT ALQUILER_CODIGO FROM DATABASERS.ALQUILER JOIN DATABASERS.PAGO_ALQUILER ON ALQUILER_CODIGO = PAGO_ALQUILER_ALQUILER
WHERE YEAR(ALQUILER_FECHA_INICIO) = YEAR(PAGO_ALQUILER_FECHA) AND MONTH(ALQUILER_FECHA_INICIO) = MONTH(PAGO_ALQUILER_FECHA)

SELECT DISTINCT ALQUILER_CODIGO FROM DATABASERS.ALQUILER JOIN DATABASERS.PAGO_ALQUILER ON ALQUILER_CODIGO = PAGO_ALQUILER_ALQUILER 


select * from DATABASERS.ESTADO_ALQUILER