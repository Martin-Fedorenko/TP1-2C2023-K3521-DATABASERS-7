...
CREATE PROCEDURE DATABASERS.CREAR_INDICES
AS
BEGIN
	CREATE UNIQUE INDEX IX_TIPO_INMUEBLE_CODIGO_DETALLE ON DATABASERS.TIPO_INMUEBLE (TIPO_INMUEBLE_CODIGO, TIPO_INMUEBLE_DETALLE);
	CREATE UNIQUE INDEX IX_ESTADO_INMUEBLE_CODIGO_DETALLE ON DATABASERS.ESTADO_INMUEBLE (ESTADO_INMUEBLE_CODIGO, ESTADO_INMUEBLE_DETALLE);
	CREATE UNIQUE INDEX IX_CANT_AMBIENTES_INMUEBLE_CODIGO_DETALLE ON DATABASERS.CANT_AMBIENTES_INMUEBLE (CANT_AMBIENTES_INMUEBLE_CODIGO, CANT_AMBIENTES_INMUEBLE_DETALLE);
	CREATE UNIQUE INDEX IX_DISPOSICION_INMUEBLE_CODIGO_DETALLE ON DATABASERS.DISPOSICION_INMUEBLE (DISPOSICION_INMUEBLE_CODIGO, DISPOSICION_INMUEBLE_DETALLE);
	CREATE UNIQUE INDEX IX_ORIENTACION_INMUEBLE_CODIGO_DETALLE ON DATABASERS.ORIENTACION_INMUEBLE (ORIENTACION_INMUEBLE_CODIGO, ORIENTACION_INMUEBLE_DETALLE);
	CREATE UNIQUE INDEX IX_CARACTERISTICA_INMUEBLE_CODIGO_DETALLE ON DATABASERS.CARACTERISTICA_INMUEBLE (CARACTERISTICA_INMUEBLE_CODIGO, CARACTERISTICA_INMUEBLE_DETALLE);
END
GO

EXECUTE DATABASERS.CREAR_INDICES
...
CREATE TABLE DATABASERS.ESTADO_ALQUILER(
	ESTADO_ALQUILER_CODIGO NUMERIC(2,0) IDENTITY PRIMARY KEY NOT NULL,
...
AS
BEGIN
	SET IDENTITY_INSERT DATABASERS ON

	INSERT INTO DATABASERS.INMUEBLE (INMUEBLE_CODIGO, INMUEBLE_NOMBRE, INMUEBLE_ANTIGUEDAD, INMUEBLE_EXPENSAS, INMUEBLE_CANT_AMBIENTES, INMUEBLE_ORIENTACION, INMUEBLE_DISPOSICION, INMUEBLE_DESCRIPCION, INMUEBLE_SUPERFICIETOTAL, INMUEBLE_TIPO_INMUEBLE, INMUEBLE_BARRIO, INMUEBLE_DIRECCION, INMUEBLE_ESTADO)
	SELECT DISTINCT INMUEBLE_CODIGO, INMUEBLE_NOMBRE, INMUEBLE_ANTIGUEDAD, INMUEBLE_EXPESAS, CANT_AMBIENTES_INMUEBLE_CODIGO, ORIENTACION_INMUEBLE_CODIGO, DISPOSICION_INMUEBLE_CODIGO, INMUEBLE_DESCRIPCION, INMUEBLE_SUPERFICIETOTAL, TIPO_INMUEBLE_CODIGO, BARRIO_CODIGO, INMUEBLE_DIRECCION, ESTADO_INMUEBLE_CODIGO 
	...
	JOIN DATABASERS.BARRIO ON INMUEBLE_BARRIO = BARRIO_DETALLE
	
	SET IDENTITY_INSERT DATABASERS OFF;
END
GO

SELECT * FROM gd_esquema.Maestra WHERE INMUEBLE_CODIGO = '142576'

SELECT * FROM DATABASERS.BARRIO JOIN DATABASERS.LOCALIDAD ON BARRIO_LOCALIDAD = LOCALIDAD_CODIGO ORDER BY 2

DROP PROC DATABASERS.MIGRAR_INMUEBLE

SELECT INMUEBLE_ORIENTACION, INMUEBLE_DISPOSICION, INMUEBLE_CANT_AMBIENTES, INMUEBLE_TIPO_INMUEBLE, INMUEBLE_ESTADO FROM gd_esquema.Maestra 
...
CREATE PROCEDURE DATABASERS.MIGRAR_SUCURSAL
AS
BEGIN
...
CREATE PROCEDURE DATABASERS.MIGRAR_ALQUILER
AS
BEGIN
	INSERT INTO DATABASERS.ALQUILER
	SELECT DISTINCT ALQUILER_CODIGO, "ALQUILER_ANUNCIO", "ALQUILER_ESTADO" ,ALQUILER_FECHA_INICIO,ALQUILER_FECHA_FIN,ALQUILER_FECHA_INICIO,ALQUILER_CANT_PERIODOS,ALQUILER_DEPOSITO,ALQUILER_COMISION,ALQUILER_GASTOS_AVERIGUA FROM gd_esquema.Maestra where ALQUILER_CODIGO IS NOT NULL
END 
GO

SELECT ALQUILER_CODIGO, PAGO_ALQUILER_CODIGO FROM gd_esquema.Maestra

SELECT ALQUILER_CODIGO FROM gd_esquema.Maestra WHERE ALQUILER_CODIGO IS NOT NULL
...
BEGIN
	...
	SELECT ALQUILER_ESTADO FROM gd_esquema.Maestra WHERE ALQUILER_ESTADO IS NOT NULL
END 
GO
...
SELECT PAGO_ALQUILER_CODIGO, ALQUILER_CODIGO, COUNT(*) FROM gd_esquema.Maestra GROUP BY PAGO_ALQUILER_CODIGO, ALQUILER_CODIGO ORDER BY COUNT(*) DESC
